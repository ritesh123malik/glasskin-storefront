...
import { mockProducts, mockCategories, Product } from "@/lib/products";
...
// ─── All categories for filter bar (includes Toners) ─────────────────────────
...
// ─── Main shop content (needs Suspense wrapper for useSearchParams) ───────────
function ShopContent() {
  ...
  const filteredProducts =
    activeFilter === "all"
      ? mockProducts
      : mockProducts.filter((p) => p.category.toLowerCase().replace(/\s+/g, "-") === activeFilter);
